...
RSpec.describe Disbursments::Generate, type: :service do
  subject { described_class.new.call }

  describe '#call' do
    context 'create disbursements for orders from last day' do
      let!(:merchant) { create(:merchant) }
      let!(:daily_orders) { create_list(:order, 10, :withing_daily_range, merchant_id: merchant.id) }
    ...
    # context 'when no orders for merchant' do; end
  end
end
